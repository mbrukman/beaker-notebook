...
public class ClojureDynamicClassLoader extends DynamicClassLoader {
  protected final ClojureLoaderProxy glp = new ClojureLoaderProxy();
  protected final List<String> paths = new ArrayList<String>();

  public ClojureDynamicClassLoader(String dir) {
    super(dir);
    glp.setOrder(70);
  ...
  class ClojureLoaderProxy extends ProxyClassLoader {

    public ClojureLoaderProxy() {
        order = 10;
        enabled = true;
    }
    @Override
    public Class<?> loadClass(String className, boolean resolveIt) {
      ...
      for(String p : paths) {
        String tpath = p + File.separator + className.replace(".", File.separator) + ".clojure";

        File f = new File(tpath);
        if(f.exists()) {
          try {
            setEnabled(false);
            System.out.println("ERROR: NOT IMPLEMENTED: Should load clojure file "+tpath);
            setEnabled(true);
            return null;
    ...
    public InputStream loadResource(String name) {
      for(String p : paths) {
        String tpath = p + File.separator + name.replace(".", File.separator) + ".clojure";
